...
\Sexpr{print(paste("rangeMapper", packageDescription("rangeMapper")$Version))}.
...
\subsection{Prepare canvas}
	...
	\begin{enumerate}
		 \item[-] Push the `COMPUTE CANVAS EXTENT' button and then select all the range polygon files (*.shp format) in order to define the global bounding box of the project.  The vector (*.shp) files for the wrens ranges data are stored in the package's directory.  Access the `help/example files' menu to get the exact pathway for this directory.
...
\bibitem[rangeMapper]{rangeMapper}
Valcu, M., et al (2012). rangeMapper: a platform for the study of macroecology of life-history traits. Global Ecology and Biogeography 21, 945-951. \url{http://rangemapper.r-forge.r-project.org}
\end{thebibliography}
\end{document}
